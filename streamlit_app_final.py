...
def download_yolo_model(model_name="yolov8n.pt"):
    ...
    model_urls = {
        ...
        "yolov11n.pt": "https://github.com/ultralytics/assets/releases/download/v11.0.0/yolov11n.pt",
        "yolov11s.pt": "https://github.com/ultralytics/assets/releases/download/v11.0.0/yolov11s.pt",
        "yolov11m.pt": "https://github.com/ultralytics/assets/releases/download/v11.0.0/yolov11m.pt",
        "yolov11l.pt": "https://github.com/ultralytics/assets/releases/download/v11.0.0/yolov11l.pt",
        "yolov11x.pt": "https://github.com/ultralytics/assets/releases/download/v11.0.0/yolov11x.pt"
    }
    ...
    if model_name in model_urls:
        ...
        try:
            with st.spinner(f"📥 Downloading {model_name} ({size})..."):
                urllib.request.urlretrieve(url, model_path)
            st.success(f"✅ {model_name} downloaded successfully!")
            return model_path
        except Exception as e:
            st.error(f"❌ Failed to download {model_name}: {e}")
            # Try using YOLO's built-in download
            try:
                with st.spinner(f"🔄 Trying alternative download for {model_name}..."):
                    model = YOLO(model_name)  # This will download automatically
                    return model_name  # Return the model name for YOLO to handle
            except Exception as e2:
                st.error(f"❌ Alternative download also failed: {e2}")
                return None
...
def load_yolo_model(model_name="yolov8n.pt"):
    ...
    try:
        # First, try to download the model file
        model_path = download_yolo_model(model_name)
...
def check_model_status():
    ...
    if available_models:
        ...
        st.warning("⚠️ No models downloaded yet. Models will be downloaded automatically when needed.")
...
def main():
    ...
    if st.sidebar.button("📥 Download All Models", help="Download all YOLO models for offline use"):
        with st.spinner("Downloading all models..."):
            all_models = ["yolov8n.pt", "yolov8s.pt", "yolov8m.pt", "yolov8l.pt", "yolov8x.pt",
                         "yolov11n.pt", "yolov11s.pt", "yolov11m.pt", "yolov11l.pt", "yolov11x.pt"]
            for model_name in all_models:
